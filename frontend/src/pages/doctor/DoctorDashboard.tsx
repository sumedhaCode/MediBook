...
export default function DoctorDashboard() {
  const myAppointments = mockAppointments.filter((a) => a.doctorId === "1");
  const upcoming = myAppointments.filter((a) => a.status === "upcoming");
  const pending = myAppointments.filter((a) => a.status === "pending");

  const stats = [
    { label: "Today's Appointments", value: "4", icon: Calendar, color: "text-primary" },
    { label: "Total Patients", value: "128", icon: Users, color: "text-primary" },
    { label: "Pending Requests", value: String(pending.length), icon: Clock, color: "text-warning" },
    { label: "This Month", value: "42", icon: TrendingUp, color: "text-success" },
  ];
              ...
              {upcoming.map((appt) => (
                <div key={appt.id} className="flex items-center justify-between p-3 rounded-lg border">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{appt.type} • {appt.date} at {appt.time}</p>
                  </div>
